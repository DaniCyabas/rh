CREATE OR REPLACE TRIGGER WKSP_WSFDC.AA_RH_TIP_DESEMPENOS
 AFTER DELETE OR UPDATE
 ON RH_TIP_DESEMPENOS
 REFERENCING OLD AS OLD NEW AS NEW
 FOR EACH ROW
DECLARE
  v_registro RH_AUDITORIAS%ROWTYPE; -- Registro de la tabla RH_AUDITORIAS
BEGIN
  v_registro.tabla := 'RH_TIP_DESEMPENOS';
  v_registro.clave := to_char( :old.cod_tip_desempeno);
