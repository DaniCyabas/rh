CREATE OR REPLACE TRIGGER WKSP_WSFDC.AA_RH_TIP_DESCUENTOS
 AFTER DELETE OR UPDATE
 ON RH_TIP_DESCUENTOS
 REFERENCING OLD AS OLD NEW AS NEW
 FOR EACH ROW
DECLARE
  v_registro RH_AUDITORIAS%ROWTYPE; -- Registro de la tabla RH_AUDITORIAS
BEGIN
  v_registro.tabla := 'RH_TIP_DESCUENTOS';
  v_registro.clave := to_char( :old.cod_tip_descuento);
  ...
  v_registro.detalle := v_registro.detalle || pae_cnf.fu_reg_auditoria( v_registro.tip_novedad, 'PRIORIDAD', :old.prioridad, :new.prioridad, 'N' );
